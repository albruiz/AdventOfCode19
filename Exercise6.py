
#parameter listaPuntos = array with paris of space's points [X,Y]
#returns listaRectas = array with the horizontal and vertical lines formed by two points
def getRectas(listaPuntos):
    verticales = []
    horizontales = []
    for i in range (len(listaPuntos)):
        x = listaPuntos[i][0]
        y = listaPuntos[i][1]

        for j in range (i, len(listaPuntos)):
            if(x != listaPuntos[j][0] and y == listaPuntos[j][1]):
                pass
            elif x == listaPuntos[j][0] and y != listaPuntos[j][1]:
                listaIntermedia = []
                listaIntermedia.append(listaPuntos[i])
                listaIntermedia.append(listaPuntos[j])
                verticales.append(listaIntermedia)
            if y == listaPuntos[j][1] and x != listaPuntos[j][0]:
                listaIntermedia = []
                listaIntermedia.append(listaPuntos[i])
                listaIntermedia.append(listaPuntos[j])
                horizontales.append(listaIntermedia)
    listaRectas = [verticales, horizontales]
    return listaRectas

#parameters = rectas1 and rectas2 are arrays that contain lines from the wires of the problem
#returns = listaIntersecciones array that contains the intersections points of the wires
def getIntersecciones(rectas1, rectas2):
    verticales1 = rectas1[0]
    verticales2 = rectas2[0]
    horizontales1 = rectas1[1]
    horizontales2 = rectas2[1]
    listaIntersecciones = []

    for i in verticales1:
        x = i[0][0]
        y1 = i[0][1]
        y2 = i[1][1]
        for j in horizontales2:
            y = j[0][1]
            x1 = j[0][0]
            x2 = j[1][0]
            if ((x1 < x and x2 > x) or (x1 > x and x > x)) and ((y1 < y and y < y2 ) or (y1 > y and y2 < y )):
                listaIntersecciones.append([x , y])
                listaIntersecciones.append(i)


    for i in verticales2:
        x = i[0][0]
        y1 = i[0][1]
        y2 = i[1][1]
        for j in horizontales1:
            y = j[0][1]
            x1 = j[0][0]
            x2 = j[1][0]
            if ((x1 < x and x2 > x) or (x1 > x and x > x)) and ((y1 < y and y < y2 ) or (y1 > y and y2 < y )):
                listaIntersecciones.append([x , y])
                listaIntersecciones.append(i)


    return listaIntersecciones

#parameter lista = array with the intersection points bettween the lines
#returns distanciaMinima = min. Manhattan distance between 2 points
def getDistanciaMin(lista):
    distanciaMinima = 10000000000000
    puntoBueno = []
    for i in lista:
        distanciaManh = abs(i[0])+abs(i[1])
        if distanciaManh < distanciaMinima :
            distanciaMinima = distanciaManh
            del puntoBueno[:]
            puntoBueno.append(i)

    return puntoBueno


def leerFichero(ruta):
    file = open(ruta, 'r')
    datos = file.readlines()
    file.close()
    vector = []
    vectorFinal = []
    for caracter in datos:
        vector.append(caracter)
    intermedio = []
    for i in vector[0].split(','):
        if (i != '\n' ):
            intermedio.append(i)
    vectorFinal.append(intermedio)
    intermedio = []

    for i in vector[1].split(','):
        if (i != '\n' ):
            intermedio.append(i)
    vectorFinal.append(intermedio)
    return vectorFinal

# parameter movimientos = array with the instructions given
# returns listaPuntos = array with the points on the space given
def obtenerPuntos(movimientos):
    listaPuntos = [[0,0]]
    for i in range(len(movimientos)):
        indicacion = movimientos[i][0]
        if(indicacion == 'R'):
            numero = movimientos[i].strip(indicacion)
            numero = int(numero)
            puntoAnterior = listaPuntos[len(listaPuntos) - 1]
            listaPuntos.append([puntoAnterior[0] + numero, puntoAnterior[1]])
        elif(indicacion == 'L'):
            numero = movimientos[i].strip(indicacion)
            numero = int(numero)
            puntoAnterior = listaPuntos[len(listaPuntos) - 1]
            listaPuntos.append([puntoAnterior[0] - numero, puntoAnterior[1]])
        elif(indicacion == 'D'):
            numero = movimientos[i].strip(indicacion)
            numero = int(numero)
            puntoAnterior = listaPuntos[len(listaPuntos) - 1]
            listaPuntos.append([puntoAnterior[0], puntoAnterior[1] - numero])
        else:#el caso en que sea Up
            numero = movimientos[i].strip(indicacion)
            numero = int(numero)
            puntoAnterior = listaPuntos[len(listaPuntos) - 1]
            listaPuntos.append([puntoAnterior[0], puntoAnterior[1] + numero])
    return listaPuntos


def getCoste(listaPuntosCable1, puntosRecta, movimientos):
    indicador = listaPuntosCable1.index(puntosRecta[0])
    coste = 0
    for i in range(indicador -1):
        numero = movimientos.strip(movimientos[i][0])
        coste += numero

    return coste


#main
ruta = 'text3.txt'
movimientos = leerFichero(ruta)
listaPuntosCable1 = obtenerPuntos(movimientos[0])
listaPuntosCable2 = obtenerPuntos(movimientos[1])
rectasCable1 = getRectas(listaPuntosCable1)
rectasCable2 = getRectas(listaPuntosCable2)
puntosInterseccion = getIntersecciones(rectasCable1, rectasCable2)
listaBuena = []
for i in range(len(puntosInterseccion)):
    if i % 2 == 0 :
        listaBuena.append(puntosInterseccion[i])
puntoBueno = getDistanciaMin(listaBuena)
indicador = puntosInterseccion.index(puntoBueno[0])
print(listaPuntosCable1, 'kll', puntoBueno)
coste1 = getCoste(listaPuntosCable1, puntosInterseccion[indicador + 1], movimientos[0])
coste2 = getCoste(listaPuntosCable2, puntosInterseccion[indicador + 1], movimientos[1])
print(coste1, coste2)
exit()
#indicador +1=puntos del cable1
#indicador +2=puntos del cable2
exit()
print(f'La distancia minima entre una interseccion y el punto (0,0) es: {distanciaMinima}' )
